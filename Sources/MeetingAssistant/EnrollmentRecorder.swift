...
/// phonetically varied paragraph (~12 s) gives the diarizer a clean voiceprint;
/// the "how to read it" instructions live in the surrounding UI, not here.
public enum EnrollmentScript {
    public static let passage = """
        The quick brown fox jumps over the lazy dog near the river bank. I usually \
        join calls from my office, sometimes from home, and occasionally while \
        travelling. My voice sounds a little different in the morning than late in \
        the day.
        """
}
...
final class EnrollmentRecorder: NSObject, ObservableObject, AVAudioRecorderDelegate {
    ...
    func record(
        to url: URL, seconds: TimeInterval = 20, completion: @escaping (Result<URL, Error>) -> Void
    ) {
        do {
