...
extension Transcribing {
    ...
    public func transcribe(audioFile: URL, channel: AudioChannel) async throws
        -> [TranscriptSegment]
    {
...
public struct StubTranscriber: Transcribing {
    ...
    public func transcribe(
        ...
        let note =
            "[transcription backend not configured — open in Xcode and add WhisperKit to enable on-device speech-to-text]"
...
// MARK: - Real engine (compiled only when WhisperKit is available)
    ...
    public actor WhisperKitTranscriber: Transcribing, LanguageDetecting {
        ...
        public func transcribe(
            ...
            let label =
                channel == .microphone ? "Transcribing your audio…" : "Transcribing others' audio…"
        ...
        private func buildPipeline(progress: TranscribeProgressHandler?) async throws -> WhisperKit
        {
            ...
            //
            // We previously forced `.cpuAndGPU` to skip the slow first-time ANE compile,
            // but on macOS 26 the GPU/MPSGraph backend aborts the whole process during
            // inference: `-[MPSGraphTensorData initWithMTLBuffer:…]` hits an MTLReportFailure
            // assertion (SIGABRT) under GPU/memory pressure. It is an Apple-framework abort
            // we can't catch, and it reproduced even with a single serialized prediction
            // (channels serialized + concurrentWorkerCount = 1), so it's the GPU compute
            // path itself, not concurrency. The Neural Engine is a different backend (the
            // MelSpectrogram model already runs on it here without issue), so it avoids the
            // crashing MPSGraph code entirely. The cost is a one-time ANE compile on first
            // use (surfaced via the "Loading model…" progress) — acceptable versus aborting.
